...
- [x] 个性化推荐：兴趣标签选择UI + 路线推荐Prompt
- [ ] 多模态：拍照上传 → Qwen-VL-Max 识别景点 → 回答
- [x] 对话历史展示（侧边栏）
- [x] 数字人"思考中"动画（LLM响应期间）

**管理后台**

- [x] 知识库管理：文件上传/列表/删除 + 处理状态显示
- [x] 数字人配置：Live2D模型选择/声音ID/系统Prompt编辑 + 实时预览
- [x] 用户登录（简单JWT，admin账户即可）
- [x] 新增独立 voice_profile 表、音频资源管理、上传/校验/试听链路，下拉选择音色

**后端**

- [x] 对话记录持久化（messages表）
- [x] 情感分析批处理任务（每日定时，LLM分析当日对话）
- [x] 感受度报告生成 API
...
- [x] 后台知识库自扩容
- [x] 口型平滑插值（避免突变，lerp过渡）
- [x] TTS 分句并行合成（LLM 流式输出 → 按句触发合成 → pipeline，降低首句延迟）
- [x] 初赛后迁移至 v100：开启 `stream=True` 流式合成，首句延迟目标 < 1.5s
- [x] v100 阶段可选：`inference_zero_shot` + 专属导游音色录音（3~10s 参考音频），进一步提升辨识度
- [x] 弱网环境降级处理（文字回复 + 简单口型）
- [x] 数字人闲置动画（无对话时轻微摇头/眨眼）
